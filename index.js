function loadImage() {
    ...
    while (flag == 0) {
		...
		if (answer.every(val => filtered.includes(val))){
			flag=1;
			document.getElementById("image").onload = function() { magnify("image", 3); }
...
function magnify(imgID, zoom) {
  if (document.getElementsByClassName("img-magnifier-glass").length>0){
	  
	var r=document.getElementsByClassName("img-magnifier-glass");
	console.log(r);
  ...
  img.parentElement.append(glass);
...
function loadTutorial() {
    ...
    for (let i = 1; i < choices.length; i++) {
        ...
        newA.id = choices[i];
		newA.className="accordion";
        newA.onclick = function () {
            document.getElementById("image").src = "./tutimages/" + choices[i] + ".jpg";
        };
		var newD = document.createElement("div");
		newD.className="panel";
        var newP = document.createElement("p");
        newP.innerHTML = text[i - 1];
		newD.appendChild(newP);
		document.getElementById("tutorialblock").insertBefore(newD, document.getElementById("tutorialblock").firstChild);
